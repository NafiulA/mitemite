...
const VideoCard: NextPage<IProps> = ({ post }) => {
    ...
    return (
        <div className='flex flex-col border-b-2 border-gray-200 pb-6'>
                ...
                <div className='flex gap-3 p-2 cursor-pointer font-semibold rounded'>
                    <div className='md:w-16 md:h-16 w-10 h-10'>
                        <Link href={`/profile/${post.postedBy._id}`}>
                            <>
                                <Image width={62} height={62} className='rounded-full' src={post.postedBy.image} alt="profile image"
                                    layout='responsive' />
                            </>
                        </Link>
                    </div>
            ...
            <div>
                <p className='lg:ml-20 ml-2 pb-3 text-md text-primary'>{post.caption}</p>
            </div>
            <div className='lg:ml-20 flex gap-4 relative'>
                <div className='rounded-3xl' onMouseEnter={() => setIsHover(true)}
                    onMouseLeave={() => setIsHover(false)}>
                    <Link href={`/detail/${post._id}`}>
                        <video ref={videoRef} src={post.video.asset.url} loop className='lg:w-[700px] h-[300px] md:h-[400px] lg:h-[530px] md:w-[500px] w-[200px] rounded-2xl cursor-pointer bg-gray-100'>
                    ...
                    {isHover &&
                        <div className='absolute bottom-6 left-8 cursor-pointer md:left-14 lg:left-0 flex gap-10 lg:justify-between w-[100px] md:w-[50px] p-3'>
                            ...
                            </button> : <button onClick={onVideoClick}>
